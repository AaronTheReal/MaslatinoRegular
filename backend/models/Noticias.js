...
const BlockSchema = new Schema({
  type: {
    type: String,
    required: true,
    enum: ['text', 'image', 'quote', 'link', 'list', 'embed', 'iframe'] // 👈 aquí agregas iframe
  },
...
BlockSchema.pre('validate', function (next) {
  switch (this.type) {
    ...
    case 'embed':
      ...
      break;
      case 'iframe':
      // Usamos el HTML completo que ya manda el frontend
      if (!this.html) {
        return next(new Error('Bloque iframe requiere html.'));
      }
      break;
  }
