...
ngOnInit() {
  ...
  if (isPlatformBrowser(this.platformId)) {
    ...
    // SSR: no cargamos nada aquí para que no truene el render
    this.loading = false;
  }

  // ⚠️ IMPORTANTE:
  // YA NO LLAMAMOS this.loadDataReal() AQUÍ
}
  ...
  // 🔒 Seguridad extra: nunca correr esto en SSR
  if (!isPlatformBrowser(this.platformId)) {
    this.loading = false;
    return;
  }
  ...
  private afterOpen() {
    ...
    // 👇 Primera vez que se abre, cargamos datos reales
    if (!this.dataLoaded) {
