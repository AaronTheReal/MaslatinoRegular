  ...
  private patchForm(noticia: Noticia) {
    ...
    noticia.content.forEach(block => {
      ...
      if (block.type === 'list' && block.items) {
        ...
        block.items.forEach(item => items.push(this.fb.control(item, Validators.required)));
      }
  ...
  private slugUniqueValidator(): AsyncValidatorFn {
    return (control: AbstractControl) => {
      ...
      return this.noticiasService.getNoticiaBySlug(control.value).pipe(
        map((noticia: Noticia | null) => noticia && noticia._id !== this.id ? { slugUnique: true } : null),
        catchError(() => of(null))
      );
    };
  }
  ...
  onDelete() {
    ...
    this.noticiasService.deleteNoticia(this.id).subscribe({
      next: () => {
        ...
        this.isSubmitting = false;
        this.router.navigate(['/noticias']); // Redirect to news list or home
      },
      error: (err: any) => {
